...
export default function Home({ setPage }) {
  ...
  useEffect(() => {
    if (!SUPABASE_URL || !SUPABASE_ANON) {
      setGallery(FALLBACK_GALLERY); setGL(false); return
    }
    fetch(SUPABASE_URL + '/rest/v1/gallery?select=*&active=eq.true&order=sort_order.asc', {
      headers: { 'apikey': SUPABASE_ANON, 'Authorization': 'Bearer ' + SUPABASE_ANON }
    })
      .then(r => r.json())
      .then(rows => { setGallery(Array.isArray(rows) && rows.length > 0 ? rows : FALLBACK_GALLERY) })
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <section style={{ background:C.white, padding:'4.5rem 3rem', borderBottom:'1px solid '+C.border }}>
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:'1.5rem' }}>
            {[
              ['Archival Print Quality',       'Acid-free 70gsm bond paper, 600dpi laser printing. Your research preserved for decades.'],
              ['24-Hour Guaranteed Delivery',  'Order before midnight. Bound thesis at your doorstep by morning.'],
              ['University Compliant Binding', 'Meets specifications of IIT, AIIMS, JNVU, MBM, DU, JNU and all Rajasthan universities.'],
              ['Transparent Pricing',          'No hidden charges. Instant quotation below — see exact cost before you upload.'],
            ].map(([title, desc]) => (
      ...
      <QuoteCalculator setPage={setPage} />
        ...
        <div style={{ maxWidth:980, margin:'0 auto' }}>
          ...
          {galleryLoading ? (
            ...
            <div style={{ textAlign:'center', padding:'3rem', border:'2px dashed '+C.border, borderRadius:8 }}>
              ...
              <p style={{ ...serif, fontSize:'1.05rem', color:C.textMuted }}>Add photos from Admin Panel → Gallery tab</p>
            </div>
          ) : (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:'1.25rem' }}>
              {gallery.map(item => (
                <div key={item.id} style={{ borderRadius:8, overflow:'hidden', border:'1px solid '+C.border, background:C.white }}>
                  <div style={{ position:'relative', height:220, background:'#fff', overflow:'hidden' }}>
                    <img src={item.image_url} alt={item.name}
                      style={{ width:'100%', height:'100%', objectFit:'contain', display:'block', padding:8 }}
                      onError={e => { e.target.style.display='none'; e.target.nextSibling.style.display='flex' }} />
                    <div style={{ display:'none', position:'absolute', inset:0, alignItems:'center', justifyContent:'center', flexDirection:'column', gap:8, background:C.gray2 }}>
                      <div style={{ fontSize:32 }}>🏛</div>
                      <div style={{ ...mono, fontSize:'0.62rem', color:C.textMuted }}>Image loading...</div>
                    </div>
                  </div>
                  <div style={{ padding:'0.85rem 1rem', borderTop:'1px solid '+C.border, background:C.gray1, textAlign:'center' }}>
                    <p style={{ ...serif, fontSize:'1.05rem', color:C.maroon, fontWeight:700, margin:0 }}>{item.name}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section style={{ background:C.white, padding:'4.5rem 3rem', borderBottom:'1px solid '+C.border }}>
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          ...
          {[
            ...
            ['Configure', 'Select binding colour and specify total pages, colour pages, copies and OHP sheets.'],
            ['Quotation', 'Instant price shown automatically — no waiting, no hidden charges.'],
            ['Payment',   'Pay via Razorpay (UPI, cards, net banking) or confirm via WhatsApp.'],
      ...
      <section style={{ background:C.gray1, padding:'4.5rem 3rem', borderBottom:'1px solid '+C.border }}>
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          ...
          <div style={{ textAlign:'center', marginTop:'2.5rem' }}>
            <button onClick={() => setPage('upload')} style={btn('primary')}>Upload Your Thesis Now</button>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))', gap:'1.25rem' }}>
            {[
              ...
              ['The binding met all IIT Jodhpur specifications without any revision. Genuinely professional.','Aakash Sharma','IIT Jodhpur'],
              ['Ordered at 11 PM, received by 8 AM. The gold embossing looked exceptionally professional.','Dr. Sunita Patel','MBM University, Jodhpur'],
            ].map(([quote, name, inst]) => (
        ...
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          <div style={{ display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'2rem', marginBottom:'2rem', paddingBottom:'2rem', borderBottom:'1px solid rgba(255,255,255,0.15)' }}>
            ...
            <div style={{ display:'flex', gap:'3rem', flexWrap:'wrap' }}>
              {[
                ['Services',['Thesis Printing','Hardbound Binding','Spiral Binding','OHP Sheets','Lamination']],
